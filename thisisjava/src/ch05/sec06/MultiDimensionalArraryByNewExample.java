...
public class MultiDimensionalArraryByNewExample {

	public static void main(String[] args) {
		// 각 반의 학생 수가 3명으로 동일 할 경우 점수 저장을 위한 2차원 배열 생성
		// 80, 83, 85  		86, 90, 92
		int[] arr1 = {80,83,85};
		int[] arr2 = {86,90,92};
		...
		System.out.println("2차원 배열의 두번째 1차원배열(첫번째 행)의 요소의 수(열)의 수: "+mathScore[1].length);
		
		//이차원 배열의 각 요소값 출력
		for(int i =0; i<mathScore.length;i++) {
			for(int j =0; j<mathScore[1].length;j++) {
		...
		System.out.println("2차원 배열의 두번째 1차원배열(첫번째 행)의 요소의 수(열)의 수: "+arr5[1].length);
		System.out.println("2차원 배열의 세번째 1차원배열(첫번째 행)의 요소의 수(열)의 수: "+arr5[2].length);
